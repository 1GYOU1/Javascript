...
const solution = (arr, k) => {
    ...
    let result = newArr.slice(0, k)
    if(newArr.length < k){
        for(let i=0;i<k-newArr.length;i++){
            result.push(-1)
        }
    }
    return result;
...
/*
    filter랑 indexOf 사용해서 중복 배열 요소들 지워주고,
    걸러진 배열에서 slice로 잘라오는데,
    길이가 짧으면 부족한 갯수만큼 for문으로 돌면서 -1 push로 넣어주기 !
*/