...
const solution = (rank, attendance) => {
    ...
    attendance.forEach((e, i) => {
        e == true ? newRankArr.push({rank: rank[i], index: i}) : ''
    })
    newRankArr = newRankArr.sort((a,b) => a.rank-b.rank).slice(0, 3)
    result = 10000 * newRankArr[0].index + 100 * newRankArr[1].index + newRankArr[2].index
    
    return result;
}

/* 
    배열에서 true(attendance)인 애들 중에 
    등수(원소 값)가 높은 순대로 3개를 뽑고,
    해당 등수 원래 배열(rank)에서의 index가 몇이었는지를 활용하여 식을 계산하는 문제.

    새 배열을 만들어서 attendance가 true인 객체(배열 값, index)를 담아주고, sort로 정렬시켜 3개만 잘라 저장.
    담아낸 0, 1, 2번째 객체 값을 이용하여 식을 만들어서 계산하여 return.
*/